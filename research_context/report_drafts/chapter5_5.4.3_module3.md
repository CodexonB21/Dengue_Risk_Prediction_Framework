...
Four design objectives guide the module. First, Stage 1 must produce an interpretable spatial baseline from district-level case intensity and geography. Second, that baseline must be checked for coherent spatial clustering rather than treated as random geographic noise. Third, Stage 2 must compensate systematic differences between observed intensity and the baseline, drawing primarily on each district's own recent case trajectory together with climate, elevation, and population covariates as secondary explanatory context. Fourth, residual adjustment must be refined through a controlled iterative update so that the final hotspot surface is a compensated spatial risk estimate rather than a one-shot residual overlay.
...
**Stage 2 — Random Forest residual compensation with iterative refinement.** Stage 2 trains a Random Forest regressor to predict the spatial residual from own-district lagged residual history, lagged rainfall and temperature features, climate anomalies, monsoon indicators, elevation, population density, and a multivariate Mahalanobis anomaly score. An early design iteration compensated using only the climate and demographic covariates; a district's own recent residual trajectory was subsequently identified as the dominant, previously-missing source of signal and added to the feature set, since dengue burden carries genuine week-to-week persistence at district level. The residual target itself is also expressed on a relative scale — the raw residual divided by the current baseline risk — after a direct diagnostic showed the raw (absolute) residual's error magnitude scales with predicted magnitude, letting the largest outbreak weeks dominate the learning signal at the expense of ordinary weeks. Unlike Modules 1 and 2, compensation is wrapped in an iterative refinement loop. At each iteration the risk surface is updated as

```text
Risk_t = Risk_(t-1) + α · predicted_relative_residual_t · (Risk_(t-1) + 1)
```

with the shrinkage factor `α = 1`, i.e. the full-magnitude update. The loop continues until successive risk changes fall below a tolerance and residual Moran’s I is no longer statistically significant, subject to a small iteration cap as a practical safeguard. An earlier, absolute-scale formulation required shrinkage (`α = 0.05`) because an unshrunk full-step update on that scale diverged during implementation; the relative-scale reformulation removed this instability, since the district's own recent relative error gives the model a stable anchor even for a held-out district under spatial cross-validation. Quantitative convergence and fit results, including the comparison against a naive persistence baseline, are reserved for Chapter 7.
...
**Notes for Team:**
- UPDATED 2026-08-08 (M3-015): Stage 2, in its final promoted form, DOES improve aggregate case-fit over Stage 1 and over a naive persistence baseline — verified via a week-level bootstrap, not just an aggregate table. Two earlier design iterations (climate-only covariates; absolute-residual own-lag features) were null or lost to the naive baseline respectively — kept as design-rationale context above, not as the final claim.
- Report Moran’s I nuance in Chapter 7: aggregated I ≈ 0.70 (significant), but NE-monsoon representative week not significant. Note also that the current Stage 2 model itself underperforms specifically at that same NE-monsoon week — an honest, still-open limitation.
- Figure assets: `research_context/report_drafts/diagrams/figure_5_5_module3_architecture.drawio` (+ `.png`) — regenerate to show the relative-residual formula, not `α = 0.05`.
