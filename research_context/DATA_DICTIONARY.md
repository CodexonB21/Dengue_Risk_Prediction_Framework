...
| **NEW: Systematic per-week date mislabeling** — 30 `(Year, Week)` labels (2008-2024) have a date stamp agreed on by almost all districts (so it is invisible to per-row/duplicate-key checks) but chronologically wrong relative to neighbouring weeks — likely a page-level MoH scrape error for that specific week, distinct from the 5 row-level collisions fixed 2026-07-26 | **Open (discovered 2026-07-27)** — needs joint team review before source correction | Full list: `data/processed/shared/epi_week_calendar_chronology_issues.csv`. Case counts for these weeks are real and unaffected; only their date stamp (and therefore climate join) is wrong. Breaks the climate day-to-week join for 15 of the 30 weeks (375/25,350 rows in `weekly_modeling_table.csv` have no matching climate for this reason; a further 125 rows have no climate for the separate, expected 2006/2026 boundary reason — left as `NaN`, not fabricated). See `module_1_forecasting/MODULE_CONTEXT.md` open question #10 and `PIPELINE_ARCHITECTURE_PLAN.md` Open Item 4 |
