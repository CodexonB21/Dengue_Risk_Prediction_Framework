...
**Key numbers:** Moran's I **0.702** (p=0.001) · own-district case history is top feature · corr(Risk,Cases) **~0.96** (beats Stage 1 ~0.82 AND naive persistence ~0.95) · 2017 Wk29 peak map
...
**[M3-2 + Fig 6.4 · 0:40]** Stage 1: **case-weighted KDE** + **Moran's I**. Stage 2: **relative spatial residual** (actual − risk, divided by risk), full-magnitude iterative update **α=1**. **5-fold spatial CV** (whole districts held out). **IDW = visualisation only**.
...
**[M3-5 + importance fig · 0:45]** Stage 2 led by **own-district case history (~67% + ~14%)** — a district's own recent burden trend — with population/climate as secondary support. Correction = **case persistence first, demographic + environmental second**.

**[M3-6 + Fig 7.5 · 0:50]** **Hero slide.** Fig 7.5: **2017 Wk29** peak — risk in **SW corridor** (**Colombo, Gampaha, Kalutara**). Matches known **2017 epidemic** geography. **corr ≈ 0.96** with observed burden — genuinely better than Stage 1 alone and a naive "no model" check.
...
- Don’t claim **sub-district** targeting  
- DO claim Stage 2 beats Stage 1 AND a naive persistence baseline (M3-015, bootstrap-confirmed) — this is now a positive result  
- Don’t show **2021 NE-monsoon** non-significant week (also Stage 2's weakest week) unless asked  
