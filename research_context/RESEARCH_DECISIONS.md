...
## Decision 031: Production "Predict Next Week" Nowcast Added; Latent Reporting-Delay Feature Bug Fixed

**Module:** Module 1
**Status:** Accepted (implemented and run, 2026-08-04)
**Date:** 2026-08-04

### Decision
Added `forecast_future.run_nowcast()`, a thin `horizon=1` wrapper around the
existing `run_future_forecast()` - at `horizon_step == 1` every feature uses
real historical values (zero recursion), so this is a genuine, not
approximate, "predict next week's case count using all data available up to
the current week" production output. Written to a new
`data/processed/module1/nowcast_next_week.csv`, wired into
`scripts/refresh_dashboard_data.py` right after the existing forecast step,
and NOT wired into `main.py` (mirrors Decision 018's separation of
forward/live outputs from the validated walk-forward pipeline). Both this
new output and the existing 8-week `future_forecast.csv` now carry an
explicit `evidence_tier` column (`operational_nowcast` for step 1,
`operational_forecast` for later steps), matching Module 2's existing
`evidence_tier` provenance-tagging convention that Module 1 previously
lacked.

While implementing this, found and fixed a **latent, pre-existing bug**:
`forecast_district()`'s explicit feature-row column list predated the
2026-07-29 M1-006B production promotion (Decision 030) and never included
the three reporting-delay columns that promotion added to production
`FEATURE_COLUMNS`. Every call to `run_future_forecast()` since that
promotion raised a `KeyError` - meaning `future_forecast.csv` (checked into
the repo, dated pre-promotion) had been silently stale and could not
actually be regenerated by the current codebase. Fixed by plumbing
`is_reporting_anomaly` history into the synthetic future-row builder and
including the three reporting-delay columns in the feature-row dict;
`future_forecast.csv` was regenerated for all 25 districts with the fix.

### Reason
The user asked for a genuine "predict next week" capability, distinct from
both the partly-recursive 8-week forecast and the historical-only rolling
evaluator. The bug was discovered only because building the nowcast required
actually exercising `run_future_forecast()` end to end for the first time
since the M1-006B promotion - it was not anticipated going in.

### Implication
- New config path `MODULE1_NOWCAST_PATH`.
- `feature_completeness_pct` for the nowcast (77-89% across districts, 2026
  Wk26) is markedly higher than the pre-fix/pre-climate-refresh 56% figure
  reported in M1-004, reflecting both the Decision 027 climate refresh and
  this fix - not a new capability, a restored one.
- The nowcast's historical accuracy is best approximated by M1-008's rolling
  `--scope all` backtest (same underlying mechanism), which carries its own
  important caveat - see Decision 032.

### Documentation Updated
`module_1_forecasting/MODULE_CONTEXT.md`, `module_1_forecasting/EXPERIMENT_LOG.md`
(M1-010), `research_context/CHANGELOG.md`.

---

## Decision 032: Higher-Power Rolling DM Test Reveals a Materially Weaker Stage 2 Benefit Under Deployment-Faithful Evaluation — Flagged, Not Resolved

**Module:** Module 1
**Status:** Accepted as a documented finding/open question (not a model or
feature change) - implemented and run 2026-08-04

### Decision
Added `rolling_one_step.compute_dm_results_rolling()`, run for the first
time across all 25 districts at `--scope all` (previously only a 2-district
spot-check existed, per M1-005) via a new parallel driver
(`scripts/run_rolling_one_step_parallel.py`, since a serial single-district
timing test projected ~3.6 hours for all 25 districts; the parallelized run
completed in ~43 minutes). This gives a per-district out-of-sample sample of
~852 weeks, versus 104 for `combine.py`'s holdout-only DM test.

**Result, reported honestly rather than framed as a simple power increase:**
only 2/25 districts reach `p < 0.05` (`Ratnapura`, `Badulla`) - and both show
Stage 2 performing **significantly worse**, not better. Only 10/25 districts
show `mean_loss_diff > 0` (Stage 2 helping at all) in this evaluation mode -
a much weaker and more mixed picture than the holdout backtest's headline
23/25-districts-improve result.

**Critical methodological caveat, identified before over-interpreting this
result**: this is **not** a like-for-like higher-powered replication of
`combine.py`'s DM test. The frozen `xgboost_final_model.json` used by the
rolling evaluator was trained on residuals computed from **fold-based**
SARIMA refits spanning almost the entire history (all 14 folds + holdout).
The rolling evaluator's **weekly-refit** SARIMA produces systematically
different `sarima_prediction` values (and downstream features) for those
same historical weeks. This DM test therefore measures something closer to
"does the fully-trained final Stage 2 model generalize under an input
distribution shift between weekly-refit and fold-refit SARIMA" than genuine
held-out generalization - conceptually the same distinction already drawn
for rolling sMAPE in `module_1_forecasting/MODULE_CONTEXT.md` ("do not cite
rolling sMAPE alongside holdout MASE as the same estimand"), now shown to
apply, more starkly than expected, to the DM test as well.

### Reason
The original critique ("DM significance is thin at the holdout") assumed
more out-of-sample data would simply sharpen the same measurement. Actually
computing it revealed the premise was incomplete: the rolling evaluator
measures a genuinely different thing (deployment fidelity under a Stage-1-
methodology mismatch), not a bigger sample of the same estimand. Surfacing
this before treating the raw numbers as a resolution is a direct application
of this project's "critique assumptions, don't just agree" mandate.

### Implication
- This does **not** replace or override `combine.py`'s holdout DM test as
  the primary significance evidence - that remains the validated
  in-scope-for-Stage-2's-own-training estimand.
- **New open question, higher priority than originally scoped**: why is
  Stage 2's benefit so much weaker under weekly-refit SARIMA inputs than
  fold-refit ones? Candidate explanations not yet investigated: (a) the
  frozen final model may be more sensitive to small SARIMA-prediction
  perturbations than the walk-forward evaluation suggests, (b) weekly refits
  may introduce more short-run SARIMA noise than annual-fold refits, (c) a
  genuine sign Stage 2 overfits fold-specific SARIMA error patterns rather
  than learning a more general correction. Resolving this is future work,
  not undertaken this session.
- `Ratnapura`/`Badulla` significantly regressing under rolling evaluation
  (despite improving in the holdout backtest) is a new, district-specific
  finding worth a closer look alongside the existing `Kilinochchi`/`Mannar`
  holdout exceptions.

### Documentation Updated
`module_1_forecasting/MODULE_CONTEXT.md` (new Open Question),
`module_1_forecasting/EXPERIMENT_LOG.md` (M1-008), `research_context/CHANGELOG.md`.

---

## Decision 033: Residual-Lag/EWMA Stage 2 Feature Extension Rejected (M1-007)

**Module:** Module 1
**Status:** Rejected (implemented, run, and rejected 2026-08-04)
**Date:** 2026-08-04

### Decision
Extending Stage 2's residual-lag features with `residual_lag_3`,
`residual_lag_4`, and a strictly-causal `residual_ewma_4` (targeting the
23/25-districts-fail-Ljung-Box finding) is **rejected** for production.
Median validation-aggregate MASE improved (0.651 -> 0.597) but median
holdout MASE regressed (0.374 -> 0.395) - a validation-improves/holdout-
regresses pattern, the same signature Decision 026 (SMOTE) used to reject
that ablation. Ljung-Box pass rate barely moved (23/25 -> 22/25 failing).

### Reason
Pre-registered adoption criterion (Ljung-Box pass rate improves AND holdout
MASE does not regress) was not met - both conditions were required
precisely to avoid adopting a change that looks good on one axis while
quietly hurting the metric that matters most (holdout accuracy).

### Implication
Code kept as a reusable ablation switch
(`compensation_model.get_feature_columns(feature_variant="m1_007_residual_ext")`,
mirrors M1-006A's precedent of keeping rejected-but-implemented code) - not
deleted, not adopted. The residual-autocorrelation gap remains open; a
different Stage 2 architecture is a more promising future direction than
more lags of the same already-dominant signal.

### Documentation Updated
`module_1_forecasting/MODULE_CONTEXT.md`, `module_1_forecasting/EXPERIMENT_LOG.md`
(M1-007), `research_context/CHANGELOG.md`, `research_context/FEATURE_ENGINEERING_SPEC.md`.

---

## Decision 034: Per-District Stage 2 Shrinkage Weight — Partial Adoption (M1-009), Kilinochchi/Mannar Unresolved

**Module:** Module 1
**Status:** Partial adopt (implemented and run 2026-08-04; not wired into
default production path)
**Date:** 2026-08-04

### Decision
Added an optional per-row `weight` parameter to
`residual_transform.combine_stage2_forecast()`
(`final = sarima + weight * predicted_residual`) and `shrinkage.py`, which
grid-searches `weight in {0, 0.25, 0.5, 0.75, 1.0}` per district using ONLY
the 14 validation folds (never the holdout), requiring a candidate to both
improve median fold MASE AND win on >= 50% of individual folds before being
preferred over the default `weight=1.0` - then confirms (never re-selects)
against the holdout per district.

**Kilinochchi and Mannar - the two districts this was built to fix - were
NOT selected for shrinkage.** Both districts' validation-fold MASE is
already improved by Stage 2 (Decision 017); their holdout regression is
concentrated in specific folds (Mannar's fold-13 explosive AR root,
Decision 017) rather than reflecting a validation-visible pattern a global
per-district weight could honestly capture. For `Mannar`, `weight=0.5`
looked attractive on median validation MASE alone (0.552 vs. 0.617) but won
on fewer than half the individual folds - exactly the overfitting pattern
the fold-win-fraction requirement exists to reject.

Two **different** districts were selected and holdout-confirmed:
`Monaragala` (`weight=0.75`, holdout MASE 0.513 -> 0.505) and `Vavuniya`
(`weight=0.75`, holdout MASE 0.365 -> 0.359) - small (~1.6%) but genuine
improvements. Applying both changes the median holdout MASE across all 25
districts not at all (only 2/25 districts affected, neither near the
median).

### Reason
A uniform shrinkage weight is the wrong tool for a fold-specific failure
mode; the honest result of a methodologically careful search is that it
finds a different, smaller win rather than the originally-hoped-for fix -
reported as such rather than forced.

### Implication
- `combine.run_combine_pipeline(apply_shrinkage=True, feature_variant=...)`
  is available (guarded to require an explicit `feature_variant`, so it can
  never silently overwrite production `combined_metrics.csv`) but not
  promoted to the default production path this session, given the small
  magnitude of benefit.
- `Kilinochchi`/`Mannar`'s holdout regression **remains an open problem**;
  a fold-specific investigation (e.g. down-weighting Stage 2 only for folds
  resembling the known explosive-AR-root pattern, rather than a district-
  wide constant) is the more promising next direction, not attempted this
  session.

### Documentation Updated
`module_1_forecasting/MODULE_CONTEXT.md`, `module_1_forecasting/EXPERIMENT_LOG.md`
(M1-009), `research_context/CHANGELOG.md`.

---

## Decision 035: Open Question #17 Root-Caused — Weekly-Refit SARIMA Instability, Not the Frozen Stage 2 Model (M1-011)

**Module:** Module 1
**Status:** Accepted as a documented root cause (not a fix) - implemented and run 2026-08-04
**Date:** 2026-08-04

### Decision
Root-caused Decision 032's open question (rolling-mode Stage 2 benefit far weaker than
holdout-mode) by testing whether re-scoring every rolling-evaluated week with the
walk-forward fold model that "actually owned" that week (instead of the single frozen
`xgboost_final_model.json`) recovers the holdout-level improvement. **It does not - it
makes the DM test's significance picture worse** (districts with Stage 2 helping: 10/25 →
8/25; DM-significant: 2/25 → 0/25). Separately, weekly-refit and fold-refit SARIMA
predictions for the same historical weeks are found to be barely correlated (mean r=0.13
across districts, several near zero or negative, e.g. `Kegalle` -0.14, `Gampaha` -0.03),
and this drift correlates positively with rolling-mode forecast error (mean r=0.26).

**Conclusion: the dominant driver is weekly SARIMA refit instability (hypothesis b),
not the frozen Stage 2 model's generalization (hypotheses a/c), which are ruled out** by
the fold-matched rescoring result.

### Reason
Decision 032 correctly declined to act on the original finding without knowing its cause.
Testing the "wrong model" hypothesis directly (via a cheap, no-retraining recombination
using `rolling_one_step.rolling_one_step_district()`'s new `sarima_prediction_overrides`/
`model_resolver` parameters) is the fastest way to falsify or confirm it - it was falsified,
redirecting attention to Stage 1's own weekly-refit stability instead of Stage 2.

### Implication
- **No model or feature is changed by this decision** - it is a diagnosis, not a fix.
- **New, actionable but NOT-yet-implemented candidate**: weekly SARIMA refitting with a
  fixed `(order, seasonal_order)` under `enforce_stationarity=False`/
  `enforce_invertibility=False` (already flagged, Decision 017, as prone to occasional
  explosive fits) may be landing on meaningfully different MLE local optima week to week,
  given how little the training window changes between consecutive refits. A more stable
  refit cadence (e.g. monthly/quarterly) or cross-refit parameter smoothing is flagged as
  future work, explicitly not attempted this session - any such change would itself need
  holdout-gated evaluation before adoption.
- **Reframes how to read the existing rolling-mode numbers** (M1-005's Colombo/Gampaha
  sMAPE, M1-008's DM test): they are real, but should be understood as measuring weekly-
  refit SARIMA's own instability as much as Stage 2's transferability - not purely a Stage 2
  weakness.
- `rolling_one_step.rolling_one_step_district()`'s new parameters are generically useful
  (not single-purpose diagnostic code) and are documented in its own docstring for reuse.

### Documentation Updated
`module_1_forecasting/MODULE_CONTEXT.md`, `module_1_forecasting/EXPERIMENT_LOG.md`
(M1-011), `research_context/CHANGELOG.md`.

---

## Decision 036: STL + ARIMA Pilot Rejected — 0/3 Districts Improved, No Wider Rollout (M1-012)

**Module:** Module 1
**Status:** Rejected (piloted, run, and rejected 2026-08-04)
**Date:** 2026-08-04

### Decision
Piloted `statsmodels.tsa.forecasting.stl.STLForecast` (STL decomposition + non-seasonal
ARIMA on the deseasonalized series, confirmed to handle seasonal/trend extrapolation
internally with no hand-rolled logic needed) on 3 confirmed non-seasonal districts
(`Colombo`, `Gampaha`, `Kurunegala`), targeting the 18/25-districts-have-no-seasonal-
SARIMA-component finding (Open Question #12). **STL+ARIMA lost on walk-forward
validation MASE for all 3 districts** (`Colombo` 1.681 vs 1.627; `Gampaha` 1.182 vs 1.055;
`Kurunegala` 0.999 vs 0.856) - no holdout check was performed for any (pre-registered
rule: only check holdout for a candidate that already won on validation). A visual
decomposition sanity check confirmed the result is not an implementation artifact - STL
genuinely extracts a real annual seasonal component and a trend tracking multi-year
outbreak cycles, but the residual remains large and spiky around outbreak weeks,
suggesting the fixed seasonal shape does not capture the dominant source of forecast
error for these outbreak-driven series.

### Reason
The pilot was deliberately scoped to 3 districts specifically to gate a wider 15-18-
district rollout on cheap, early evidence before committing to the much larger blast
radius of changing any district's production Stage 1 config (which would require a full
Stage 2 retrain for that district, per Decision 017's precedent). A 0/3 result is exactly
the stopping condition the pilot was designed to catch.

### Implication
- No production Stage 1 config changed for any district.
- `src/module1_forecasting/stl_arima.py` kept as a candidate module, not deleted, in case
  a different configuration or district subset is worth revisiting later - not planned
  this session.
- Adds a fuller explanation to Open Question #12: the 18/25 non-seasonal-SARIMA finding
  is likely not simply an `auto_arima` search limitation fixable by any seasonal-
  decomposition method - consistent with, and reinforcing, M1-002's existing finding that
  Stage 2's own seasonal/monsoon/climate features compensate for this gap more
  effectively than reworking Stage 1's seasonality handling would.
- A real implementation bug was found and fixed during this pilot (not a research
  finding): `STLForecast`'s underlying `ARIMA` model rejects a constant trend term when
  `d > 0` (differencing already eliminates a constant) - fixed by conditioning
  `trend="c"` on `order[1] == 0`.

### Documentation Updated
`module_1_forecasting/MODULE_CONTEXT.md`, `module_1_forecasting/EXPERIMENT_LOG.md`
(M1-012), `research_context/CHANGELOG.md`.

---

## Decision 037: SARIMA Warm-Starting Rejected — No Effect on Prediction, Real Performance Cost (M1-013)

**Module:** Module 1
**Status:** Rejected (implemented, tested, and rejected 2026-08-04)
**Date:** 2026-08-04

### Decision
Tested whether seeding each week's rolling SARIMAX refit with the previous week's
converged parameters (`start_params`) stabilizes weekly-refit predictions, targeting
Decision 035's hypothesized mechanism (the optimizer landing on different local optima
week to week). Added backward-compatible `start_params`/`return_params` parameters to
`baseline_sarima.fit_and_forecast()` and a `warm_start` parameter to
`rolling_one_step.rolling_one_step_district()` (default behavior unchanged for both).

A cheap 60-week, 2-district check (`Kilinochchi`, `Colombo`) - run before committing to
a full 25-district evaluation - found warm-started and cold-started predictions are
**virtually identical** (e.g. Colombo: 80.250588 cold vs 80.251638 warm; correlation
with fold-refit SARIMA unchanged to 4 decimal places for both districts), while
warm-started fits took **5-10x longer per fit** (2.4-10s vs ~0.6s). Given this fully
falsified the hypothesis with no ambiguity, the full 25-district run (which would have
cost ~2+ hours given the discovered per-fit slowdown, well beyond the ~40 minutes
originally estimated) was deliberately not run - the smaller sample already answered the
question.

### Reason
The optimizer converges to essentially the same solution regardless of starting point
for this model/data combination - it was already robust to initialization, just less
efficient when given an explicit one instead of its own internal heuristic. This rules
out "optimizer local optima" as the mechanism behind Decision 035's finding and sharpens
it: the instability is a property of how the true MLE optimum itself shifts as the
training window grows by one observation each week, not an artifact fixable by seeding
the search differently.

### Implication
- No change to any production code path (`warm_start=False` is the only path used
  anywhere by default).
- The underlying weekly-refit-instability question (Decision 035) remains open. Untried
  candidates for future work: a less frequent refit cadence (e.g. monthly, holding
  parameters fixed for intermediate weeks) or smoothing/ensembling forecasts across the
  last few independent weekly refits - neither attempted this session.
- `start_params`/`return_params` are kept as generic, reusable infrastructure (harmless,
  opt-in) even though the specific ablation they were built for failed.

### Documentation Updated
`module_1_forecasting/MODULE_CONTEXT.md`, `module_1_forecasting/EXPERIMENT_LOG.md`
(M1-013), `research_context/CHANGELOG.md`.

---

## Decision 038: Less-Frequent SARIMA Refit Cadence Rejected — Cheaper but No Stability Gain (M1-014)

**Module:** Module 1
**Status:** Rejected (implemented, tested on a cheap sample, and rejected 2026-08-04)
**Date:** 2026-08-04

### Decision
Tested refitting SARIMA every 4 weeks instead of every week, using
`SARIMAXResults.append(refit=False)` (confirmed via the statsmodels docstring to be a
cheap state update, not a re-optimization) to incorporate new observations between
refits. On the same 60-week/2-district sample used for Decision 037's warm-start check,
this was ~4x cheaper (as expected) but showed no meaningful improvement in correlation
with fold-refit SARIMA predictions (`Colombo`: 0.1809 cold vs 0.1833 low-freq;
`Kilinochchi`: both statistically indistinguishable from zero).

### Reason
`.append(refit=False)` by construction keeps the parameters from the last cold refit
fixed - it cannot correct for whatever caused that refit's own parameters to already
differ from the fold-refit vintage's. This adds to Decision 037's finding: the
instability is in what a single fit's true optimum is for a given window, not in refit
frequency or optimizer seeding.

### Implication
`rolling_one_step._low_freq_refit_step()` kept as working, reusable infrastructure (a
genuine cost-saving refit-cadence control) but not pursued further - no full 25-district
run was performed, since the cheap sample gave an unambiguous negative signal.

### Documentation Updated
`module_1_forecasting/MODULE_CONTEXT.md`, `module_1_forecasting/EXPERIMENT_LOG.md`
(M1-014), `research_context/CHANGELOG.md`.

---

## Decision 039: Vintage-Ensembled SARIMA Prediction Accepted — First Broad Rolling-Mode Accuracy Improvement (M1-015)

**Module:** Module 1
**Status:** Accepted (implemented, tested at full 25-district scale, and accepted 2026-08-04;
promoted to the production nowcast 2026-08-05, see Decision 040)
**Date:** 2026-08-04

### Decision
Averaging, in transformed space, each week's fresh SARIMA forecast for target week *t*
with the last 3 weeks' own independently-fitted models' forecasts for the SAME week *t*
(each extended forward via a cheap `.forecast(steps=h)` call - no refitting, no extra
optimization cost) is **accepted** as a genuine rolling-mode accuracy improvement.
Full 25-district results: districts with Stage 2 helping in rolling mode rose from 10/25
(single-vintage baseline, M1-008) to **24/25**; DM-significant districts rose from 2/25
to 3/25 (`Colombo` narrowly missed at `p=0.052`); rolling sMAPE improved for 22/25
districts (median 58.8% → 56.8%, with several districts improving 10-15%). Only the three
most extreme zero-inflated districts (`Kilinochchi`, `Mannar`, `Mullaitivu`) got
marginally worse, by under 1 percentage point each.

### Reason
Each week's fresh MLE fit carries real, partly-independent noise (Decisions 035/037's
finding that the true optimum itself shifts week to week, not an optimizer-seeding
artifact). Averaging several independent fits' opinions about the same target week is
the standard forecast-combination remedy for exactly this kind of noise, and unlike the
rejected warm-start approach (Decision 037), it costs nothing extra - the vintage models
being averaged are ones the rolling evaluator needs to fit anyway for later weeks.

### Implication
- This is the first genuine, broad-based accuracy improvement found across the entire
  M1-007 through M1-015 investigation arc into Module 1's critique points.
- **Directly relevant to the production "predict next week" nowcast**
  (`forecast_future.run_nowcast()`, Decision 031), which currently uses a single SARIMA
  refit and therefore does not yet benefit from this finding. Recommended next step:
  extend the nowcast to construct a small ensemble of retrospective refits (fit on data
  through each of the last few available weeks, each forecasting forward to the same
  target week), mirroring `rolling_one_step._vintage_ensemble_step()`'s logic - not
  implemented this session pending explicit sign-off, since it touches the production
  nowcast path.
- New reusable function `rolling_one_step._vintage_ensemble_step()` and driver
  `scripts/run_rolling_one_step_ensemble_parallel.py`. No existing production default
  path changed - the ensemble evaluation writes to `_ensemble`-suffixed artifacts only.

### Documentation Updated
`module_1_forecasting/MODULE_CONTEXT.md`, `module_1_forecasting/EXPERIMENT_LOG.md`
(M1-015), `research_context/CHANGELOG.md`.

---

## Decision 040: Vintage-Ensembled SARIMA Promoted to the Production Nowcast (M1-016)

**Module:** Module 1
**Status:** Accepted (implemented and run 2026-08-05)
**Date:** 2026-08-05

### Decision
`forecast_future.run_nowcast()` - the production "predict next week using all data up to
now" pathway (Decision 031) - now defaults to the vintage-ensembled Stage 1 prediction
validated in Decision 039/M1-015, via a new `_ensembled_next_week_sarima()` helper and a
new `MODULE1_NOWCAST_ENSEMBLE_WINDOW = 4` config constant. Each vintage is refit fresh
(the rolling evaluator's persisted-state reuse doesn't apply to a stateless one-off call),
costing `ensemble_window` SARIMA fits instead of 1 - still cheap for a script that isn't
run in a tight loop. `forecast_district()`/`run_future_forecast()` gained an
`ensemble_window` parameter defaulting to `None` (unaffected 8-week recursive path;
ensembling is scoped to the genuine single-step nowcast only). A new `n_sarima_vintages`
output column reports how many vintages actually succeeded per row.

Verified backward-compatible: `ensemble_window=None` reproduces the pre-existing
single-fit nowcast numbers exactly (checked for `Colombo`/`Kilinochchi` to the last
decimal). Production `nowcast_next_week.csv` regenerated for all 25 districts;
`n_sarima_vintages == 4` for every district for the current target week.

### Reason
Decision 039 validated this mechanism at full 25-district scale inside the rolling
evaluator (Stage-2-helps districts 10/25 → 24/25; rolling sMAPE improved for 22/25
districts) - a direct, low-risk production application of an already-validated result,
not a new modeling risk.

### Implication
- **Does not change the core Stage 1 → Stage 2 architecture.** Stage 2 and the additive
  combination formula are untouched; only how Stage 1's single number is computed for the
  nowcast changes.
- **Does not touch the validated walk-forward/holdout backtest pipeline** (`main.py`,
  `combine.py`, `baseline_sarima.py`'s own selection/validation code) - the headline
  holdout MASE numbers are unaffected. Consistent with keeping forward/operational code
  structurally separate from that pipeline (Decision 018).
- CLI: `python -m src.module1_forecasting.forecast_future --nowcast [--ensemble-window N]`
  (`0` disables ensembling, reverting to the single-fit path).

### Documentation Updated
`module_1_forecasting/MODULE_CONTEXT.md`, `module_1_forecasting/EXPERIMENT_LOG.md`
(M1-016), `research_context/CHANGELOG.md`.

---

## Decision 041: Prospective Nowcast Accuracy Tracking Added (M1-017)
...
### Decision
Added `src/module1_forecasting/nowcast_tracking.py`: `append_to_nowcast_log()` (wired into
`run_nowcast()`, default on) appends every nowcast prediction to a permanent, append-only
log (`nowcast_prediction_log.csv`); `reconcile_nowcast_log()` (wired into
`scripts/refresh_dashboard_data.py` as a new `module1_nowcast_reconcile` step) joins that
log against `weekly_modeling_table.csv`'s real case counts wherever a logged target week
has since resolved, writing `nowcast_prospective_accuracy.csv`. Seeded with the 25-row
2026 Wk26 nowcast generated under Decision 040; first reconciliation correctly shows
0/25 resolved (that week hasn't happened yet).

### Reason
M1-016 (Decision 040) promoted a change to the production nowcast whose real-world
accuracy cannot be checked against any existing evidence source - every other Module 1
metric (walk-forward validation, the 104-week holdout, the rolling evaluator) scores
against data already in the dataset. The only honest way to get genuinely prospective
evidence is to log predictions now and wait for reality to catch up.

### Implication
- Pure infrastructure - does not itself demonstrate whether Decision 040 helped. The
  `nowcast_prospective_accuracy.csv` table will grow additively as real weeks resolve and
  should be checked periodically, not assumed silent.
- Not a substitute for, and should not be conflated with, M1-015's rolling-evaluation
  evidence - a different, slower-arriving, but genuinely prospective evidence tier.
- No existing Module 1 pipeline, model, or default artifact affected beyond `run_nowcast()`
  gaining a side-effect (the log append) and `refresh_dashboard_data.py` gaining one more
  step.

### Documentation Updated
`module_1_forecasting/MODULE_CONTEXT.md`, `module_1_forecasting/EXPERIMENT_LOG.md`
(M1-017), `research_context/CHANGELOG.md`.

---

## Decision 042: Robust Ensemble Aggregation (Median/Trimmed-Mean) Tested and Not Promoted (M1-018)

**Module:** Module 1
**Status:** Tested, not promoted - `aggregation="mean"` remains the production default (2026-08-05)
**Date:** 2026-08-05

### Decision
Extended the vintage ensemble (Decision 040) with a selectable `aggregation` rule
(`"mean"`, `"median"`, `"trimmed_mean"`) and re-ran the retroactive spot-check at a larger
scale (600 (district, week) pairs - 25 districts x 24 recent weeks, up from 175) to both
(a) get a cleaner read on the improvement's real size, and (b) test whether a
robust-to-outlying-vintages aggregation reduces the individual-week noise seen in the
first spot-check.

**Result**: the larger sample confirms a real but more modest improvement (median
absolute error -10%, mean -10%, individual-week win rate 56.8% - down from the smaller
sample's noisier 59%, not a sign the effect weakened). **Median and trimmed-mean do not
meaningfully outperform plain mean** (median abs error 11.75 vs. mean-aggregation's
11.70; mean abs error 41.95 vs. 42.41 - a ~1% difference). At `ensemble_window=4`, median
and trimmed-mean are mathematically identical (both reduce to averaging the middle two
sorted values), so this only tested two genuinely distinct rules, not three.

### Reason
The hypothesis was that a single outlying vintage fit was driving individual-week noise,
and a robust statistic would suppress it. The evidence doesn't support this: the
remaining per-week variance looks like genuine forecast uncertainty inherent to the
target week (real epidemiological volatility), not an artifact of how the 4 vintage
forecasts get combined - no combination rule over the same 4 fits can average that away.

### Implication
- `aggregation="mean"` remains the production default in `run_nowcast()` - implemented
  and available (`aggregation="median"`/`"trimmed_mean"`) but not promoted, since neither
  shows a real advantage at this sample size.
- `Mannar` is a clean, complete loser regardless of aggregation rule (0/24 weeks) -
  confirms this is a district-specific Stage 1 fit-instability problem (Decision 017),
  not something any aggregation choice was ever going to fix.
- `forecast_future.py`'s new `_collect_vintage_forecasts()`/`_aggregate_vintage_forecasts()`
  split and `forecast_district()`'s `precomputed_sarima_forecast` override are kept as
  reusable infrastructure regardless of this specific result.

### Documentation Updated
`module_1_forecasting/MODULE_CONTEXT.md`, `module_1_forecasting/EXPERIMENT_LOG.md`
(M1-018), `research_context/CHANGELOG.md`.

---

## Decision 043: Reporting-Anomaly Leakage Pathway Found, Verified Not to Have Inflated Decision 030; Real-Time Dip-Adjustment (Option A) Ruled Out (M1-019)

**Module:** Module 1
**Status:** Leakage documented (no retraction needed - verified not material); real-time
point-forecast adjustment (Option A) rejected on precision grounds (2026-08-06)
**Date:** 2026-08-06

### Decision
While scoping the reporting catch-up spike problem, found that `reporting_anomalies.
flag_reporting_anomalies()` (Decision 026/028) needs `cases[i+1]` to flag week *i* - so
every downstream use of week *T-1*'s flag as a feature/mask for predicting row *T*
(`feature_engineering.build_fold_agnostic_features()`'s nowcast correction and Feature
Group 6; `compensation_model.build_residual_lags()`'s residual-lag masking) is, subtly,
informed by `cases[T]` itself. Built a leakage-closed variant using a new causal
detector (`flag_reporting_dip_causal()`, drop-only, no rebound confirmation) in place of
the retrospective flag everywhere it currently feeds masking/features, and re-ran the
full Stage 2 + combine pipeline (`scripts/evaluate_reporting_leakage_fix.py`,
`feature_variant="causal_safe"`). **Result: median holdout MASE 0.3655 (causal-safe) vs.
0.3741 (production) - the leakage-closed variant is not worse, if anything slightly
better (-2.3%).** Separately, characterized the causal detector's real-time usability
(`scripts/evaluate_causal_dip_detector.py`): 100% recall but only 42.9% precision
against the retrospective flag as ground truth, worse still for `Colombo` (46.2%) and
`Gampaha` (30.0%) specifically - the two districts an adjustment would matter most for.

### Reason
The leakage pathway is real and independently verified in code, but the re-run shows it
did not inflate Decision 030's reported improvement - closing it doesn't hurt (and
slightly helps) accuracy, so no retraction of that decision is warranted. Separately, the
causal detector's precision is far too low (worst exactly where it would matter most) to
support building any point-forecast adjustment on top of it (Option A from the scoping
plan, `virtual-bouncing-fiddle.md`) - more than half of real-time flags in Colombo/Gampaha
would be false alarms on a genuine decline, which an adjustment would then actively make
worse.

### Implication
- Decision 030/M1-006B's production promotion stands unchanged - this is a documented
  integrity check, not a correction.
- **Option A (real-time point-forecast adjustment for reporting catch-up spikes) is
  rejected** - the precision numbers rule it out directly.
- **Option B (attach a `possible_reporting_dip` uncertainty flag to the nowcast, no
  change to any point prediction) remains open, low-risk future work** - not built this
  session, since the ask was to scope with real numbers before building anything, not to
  build speculatively.
- New reusable infrastructure kept regardless: `flag_reporting_dip_causal()`,
  `assemble_stage2_table()`/`run_stage2_pipeline()`'s `weekly_table_path`/
  `feature_table_path` overrides (both additive, production defaults unchanged).

### Documentation Updated
`module_1_forecasting/MODULE_CONTEXT.md`, `module_1_forecasting/EXPERIMENT_LOG.md`
(M1-019), `research_context/CHANGELOG.md`.

---

## Decision 044: XGBoost Hyperparameter Search Rejected — Winning Validation Candidate Failed the Holdout Check (M1-020)

**Module:** Module 1
**Status:** Rejected (searched, one candidate holdout-checked, and rejected 2026-08-06)
**Date:** 2026-08-06
...
scored via `combine.compute_district_fold_metrics()` on walk-forward folds 2-14 only -
the exact function and metric production already publishes. The harness was verified to
reproduce the published baseline (0.5821 validation-aggregate median MASE) exactly before
trusting any candidate score, after catching and fixing a missing 0-floor clip. 5/39
candidates cleared a pre-registered overfitting safeguard (beat baseline AND a majority
of the 13 folds AND a majority of the 25 districts) - the best reached 0.5659 (-2.8%).
**The one-time holdout check on that single best candidate showed a regression**: median
holdout MASE 0.3874 vs. production's 0.3741 (+3.6%). Per the pre-registered rule, no
other qualifying candidate was checked against holdout.

### Reason
A hyperparameter set that broadly beat the production defaults across 13 validation
folds - clearing a safeguard specifically built to filter out fold-count noise - still
failed to generalize to the genuinely held-out block. This is not a flaw in the search or
the safeguard; it demonstrates that even a majority-vote requirement across 13-14 folds
does not guarantee holdout generalization, which is exactly why Decision 009's "holdout
touched once, never for selection" rule exists. Checking further candidates after the
first failure would have defeated that same discipline.

### Implication
- Production `XGB_BASE_PARAMS` unchanged - the original defaults were already reasonable
  for this project's regime.
- Additive, backward-compatible `xgb_params` overrides on `train_and_predict_fold()`/
  `train_and_predict_holdout()`/`_fit_with_early_stopping()` are kept as reusable
  infrastructure (default `None` reproduces production exactly) - no further search
  planned this session.
- This is the sixth consecutive rejected ablation this session (M1-013, M1-014, M1-018's
  aggregation tuning, the in-holdout ensemble check, M1-019's real-time adjustment, and
  now this) against the one accepted improvement (M1-015/M1-016) - a pattern worth stating
  plainly rather than continuing to search for a win that may not be there at this level
  of the pipeline.

### Documentation Updated
`module_1_forecasting/MODULE_CONTEXT.md`, `module_1_forecasting/EXPERIMENT_LOG.md`
(M1-020), `research_context/CHANGELOG.md`.

---

## Decision 045: Per-District Stage 2 Rejected — Pooling Confirmed Correct by Direct Ablation (M1-021)

**Module:** Module 1
**Status:** Rejected (implemented, evaluated, and rejected 2026-08-06)
**Date:** 2026-08-06
...
`District` categorical), fold structure, and evaluation method fixed - a single-variable
test. A three-tier data-sufficiency rule (no-op below 104 trainable rows, fixed tree
count 104-207, early stopping at 208+) protected against fitting models to token amounts
of per-district data. **Result: decisively worse** - validation-aggregate median MASE
0.7473 vs. the pooled baseline's 0.5821 (+28.4%), with only 1/13 folds and 4/25 districts
improving. No holdout check performed (safeguard not cleared). The 4 districts that DID
improve (`Monaragala`, `Mannar`, `Vavuniya`, `Matale`) are ones already flagged (Decisions
017/034/037) as districts where the pooled model's correction underperforms - a coherent
pattern, not noise.

### Reason
This directly confirms, with ablation evidence rather than a-priori reasoning, why
Decision 002/014 chose pooling: most districts (21/25) depend on the shared learning
pooling provides, and lose far more than they gain without it, even with explicit
protection against under-trained per-district models.

### Implication
- Stage 2 remains pooled - no change to production.
- Sharpens Decision 037's shrinkage work: the districts where per-district modeling wins
  are specific, already-diagnosed cases (not a general pattern), suggesting a *targeted*
  per-district remedy for those few districts specifically is more promising than a
  wholesale architecture change - flagged as a possible narrow follow-up, not built this
  session.
- New reusable, additive infrastructure kept regardless: `train_and_predict_fold_per_district()`/
  `train_and_predict_holdout_per_district()` (`compensation_model.py`), and a latent bug
  fix in `_prepare_xy()` (only categorical-encodes `District` when present in
  `feature_columns` - harmless until this experiment needed a feature set without it).
...
## Decision 052: Module 3 Forward Hotspot Forecast Citation Corrected; Prospective Accuracy
Tracking Added (M3-016)
...
### Decision
Two related fixes, found together while auditing the dashboard for consistency:

1. **Citation correction**: `forecast_future.py`'s module docstring, `module_3_spatial/MODULE_CONTEXT.md`'s
   "Forward Operational Hotspot Forecast" section, `src/dashboard/views/operational_monitoring.py`'s
   "How this map is built" expander, and the 2026-08-08 Figure 5.1 changelog entry all cited
   "Decision 031" as the decision authorizing Module 3's forward hotspot forecast / M1→M3
   operational integration. Decision 031 is actually an unrelated Module 1 entry ("Production
   'Predict Next Week' Nowcast Added") with no mention of Module 3 — no decision documenting the
   Module 3 forward forecast itself had ever been written. This entry is that decision, backfilled,
   and every citing location above now points to it instead.
2. **Prospective accuracy tracking added**: new `src/module3_spatial/hotspot_tracking.py`
   (`append_to_hotspot_log()` / `reconcile_hotspot_log()`), mirroring Decision 041/M1-017's nowcast
   tracker and Decision 048/M2-015's forward-risk tracker. `forecast_future.run_forecast_future()`
   now logs every forecast row by default (`log_prediction=True`) to
   `data/processed/module3/hotspot_prediction_log.csv`. Reconciliation, once a logged target week's
   real case count is reported, recomputes what Stage 1's KDE baseline WOULD have been using the
   real total case count (the same `compensation_model.rescale_kde_baseline()` mass-conservation
   every other Module 3 evidence source already uses) and reapplies the already-logged
   `predicted_relative_residual` unchanged — Stage 2's Random Forest only ever consumes
   backward-looking lag/climate/static features, never this week's own case count, so it does not
   need recomputing. Output: `outputs/metrics/module3/hotspot_prospective_accuracy.csv`, with both
   `abs_error` (total forecast error) and `risk_0_abs_error` (the portion inherited specifically
   from Module 1's case-count forecast feeding Stage 1's KDE mass). Wired into
   `scripts/refresh_dashboard_data.py` as `module3_forecast_future` + `module3_hotspot_reconcile`,
   and surfaced as a third tracker panel on the dashboard's Prospective Tracking page.

### Reason
Found while auditing the dashboard for the same class of staleness Decision 047/050 already caught
elsewhere: a citation that quietly pointed at the wrong decision is a real credibility risk in a
viva if an evaluator looks it up. Separately, the Prospective Tracking page showed trackers for
Module 1 and Module 2 only, with no explanation for Module 3's absence — initially assumed to be an
open research question (what does "resolving" a continuous spatial risk forecast even mean?), but on
inspection this is not actually unsolved: because Stage 2's lag features are always backward-looking,
the only quantity that differs between a Module 3 forecast and its eventual real outcome is Stage
1's KDE baseline, which only needs the real case count once available — the same shape of problem
M1/M2 already solved, not a new one.

### Implication
- No production model, metric, or default artifact changed for Module 3's Stage 1/Stage 2 — this
  is new tracking infrastructure plus a documentation fix, not a retrain.
- A second, previously undocumented gap was found and fixed while wiring this in:
  `scripts/refresh_dashboard_data.py`'s `run_refresh()` never actually called
  `src.module3_spatial.forecast_future` at all, despite `MODULE_CONTEXT.md` and `DASHBOARD_GUIDE.md`
  both stating it was wired in — meaning `future_hotspot_forecast.csv` could silently go stale on
  every dashboard refresh. Added as `module3_forecast_future`, ordered after `module1_forecast_future`
  (its only dependency) and before the Module 2 steps, per the already-documented intended order.
- Module 3's forecast (and therefore this new tracker) is currently limited to
  `DEFAULT_HORIZON_WEEKS=1` — a pre-existing, separately documented limitation
  (`forecast_future.py`'s own docstring), not something this decision changes.
- 0 resolved rows is expected immediately after this change, for the same honest reason M1/M2's
  trackers showed 0 resolved when first added (Decision 041/048).

### Documentation Updated
`module_3_spatial/MODULE_CONTEXT.md` (citation fix; new tracker section),
`module_3_spatial/EXPERIMENT_LOG.md` (M3-016), `research_context/CHANGELOG.md` (this entry),
`src/dashboard/DASHBOARD_GUIDE.md`, `research_context/EVALUATION_STUDY_PLAN_DASHBOARD.md`.

---

## Decision 053: Module 1/2 Forward Horizon Shortened From 8 to 4 Weeks — Recursive
Residual-Correction Bias Found and Named, Not Fixed (M1-022/M2-017)
...
### Decision
Shortened the shared `FORECAST_HORIZON_WEEKS` constant (`src/config.py`) from 8 to 4. This
constant already drove Module 2's `forecast_future_risk.py` correctly (imported from config), but
Module 1's `forecast_future.py` had its own, separately hardcoded `FORECAST_HORIZON_WEEKS = 8` -
never actually reading config.py's same-named constant - so it is now made to import from
`src.config` instead, closing a real "same value defined twice, silently able to drift" gap
(the exact pattern already flagged as a past incident in `DASHBOARD_GUIDE.md`, re: Decision 047's
threshold going stale in a duplicated location).

The horizon change followed a direct data investigation, prompted by the user noticing the 8-week
case forecast appeared to decline in nearly every district. Decomposing `future_forecast.csv` into
its two components confirmed this is a real, systematic pattern, not noise: SARIMA (Stage 1) stays
essentially flat across the full 8-week horizon (e.g. Colombo's Stage 1 baseline moved only
335.1-367.8 across all 8 steps), while Stage 2's XGBoost residual correction declines steadily and
substantially (Colombo: 202→278→207→204→176→161→146→119; Kilinochchi's correction goes
increasingly negative: 5.8→0→‑4→‑4→‑5→‑7→‑8→‑13). The mechanism is visible directly in the data:
`residual_lag_1_is_recursive` is `False` only at horizon_step=1 and `True` for every step after -
from week 2 onward Stage 2 is feeding on its OWN prior predictions instead of real data, and small
directional errors compound step over step instead of averaging out (a known instability of
recursive multi-step forecasting with tree-based lag-feature models, distinct from SARIMA's own
multi-step math, which correctly reverts to a stable mean). Prior to this investigation, the
project's own documentation (`forecast_future.py`'s comment, `DASHBOARD_GUIDE.md`'s Known
Limitations) already named "feature completeness declines" and "uncertainty grows with horizon" as
limitations, but had not characterized this as a *systematic directional bias* - a materially
stronger and more precise claim than "less confident."

### Reason
24 of 25 districts declined from horizon_step=1 to horizon_step=8 in `final_prediction` (four -
Kilinochchi, Mannar, Vavuniya, Mullaitivu - collapsed to exactly 0), and Module 2's forward risk
showed a related rise-then-fall shape (it inherits Module 1's forecasted case counts as a lag
feature from horizon_step≥2, per Decision 027, so it lags one step behind Module 1's own drift
before following it down). Shortening the horizon does not fix the recursive bias itself - it is
already partly visible by horizon_step=3-4 for the more fragile districts - but it meaningfully
reduces exposure to its worst effects while keeping a still-actionable lead time: feature
completeness at step 4 is ~64% vs. ~47% at step 8, and only 2 of 25 districts now collapse to
exactly 0 (down from 4). A full fix (training separate direct per-horizon models instead of one
model applied recursively - the standard remedy for recursive-forecast bias in the ML forecasting
literature) was considered and explicitly NOT attempted now: it is a real modeling change requiring
its own walk-forward validation per horizon, and introducing it this close to a defense without
that validation would risk exactly the kind of unvalidated change this project's own guardrails
warn against (see Decision 044/045's "validation win didn't survive holdout" pattern).

### Implication
- No production model weights changed for either module - this is a horizon-parameter change plus
  a documentation/consistency fix, not a retrain.
- Regenerated `data/processed/module1/future_forecast.csv` (100 rows = 25 districts × 4 weeks, was
  200), `data/processed/module2/future_risk_predictions.csv` (125 rows = 25 × (1 observed + 4
  forward), was 225), and Module 3's forward forecast/prospective log (unaffected in row count -
  M3 only ever reads Module 1's horizon_step=1 row - but regenerated for consistency).
- Dashboard text updated (`operational_monitoring.py`'s nowcast panel caption and case-forecast
  chart title, `data_loaders.py`'s `load_m1_nowcast()` docstring) - all previously said "8-week."
- The recursive-bias finding itself is **not** fixed, only its worst-case exposure reduced - flagged
  directly here and in the experiment logs as future work (direct per-horizon modeling), rather than
  quietly narrowed away.

### Documentation Updated
`src/config.py`, `src/module1_forecasting/forecast_future.py`,
`module_1_forecasting/EXPERIMENT_LOG.md` (M1-022), `module_2_classification/EXPERIMENT_LOG.md`
(M2-017), `src/dashboard/DASHBOARD_GUIDE.md`, `research_context/CHANGELOG.md` (this entry).
